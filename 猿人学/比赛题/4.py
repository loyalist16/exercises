...
def run(page):
    ...
    headers = {
        ...
        'Cookie': 'Hm_lvt_0362c7a08a9a04ccf3a8463c590e1e2f=1612338356,1612423240,1612423439,1613720198; Hm_lvt_c99546cf032aaa5a679230de9a95c7db=1612230816,1613720201; qpfccr=true; no-alert=true; Hm_lvt_9bcbda9cbf86757998a2339a0437208e=1612234326,1613720260; Hm_lpvt_0362c7a08a9a04ccf3a8463c590e1e2f=1613720570; tk=-3884792864035405662; sessionid=ovmhbjgjxlnl4amju5d4qeych0pkxypk; m=a32d835f1c785fdcdaccfd225bd337b7|1613720943000; Hm_lpvt_9bcbda9cbf86757998a2339a0437208e=1613724752; Hm_lpvt_c99546cf032aaa5a679230de9a95c7db=1613724902'
    }
    ...
    for td in tds:
        ...
        for img in td.find_all('img'):
            if j_key in img.attrs['class']:
                continue
            number = number_mapping[img.attrs['src'].split(',')[1]]
            ind = int(re.search('-?\d+', img.attrs['style']).group()) // 11.5
            # print(number, ind)
            n_list.append((number, int(ind)))
        ...
        for i, v in enumerate(n_list):
            ...
            if ans[pos] != 0:
                if int(v[1]) < 0:
                    ...
                    ans.insert(pos + 1, v[0])
            else:
                ans[pos] = v[0]
        while 0 in ans:
            ans.remove(0)
        ans_list.append(int(''.join(ans)))
        # print('----')
    print(page, ' : ', ans_list)
    return ans_list


def get_j_ket(key, value):
    url = f'http://127.0.0.1:3000/?key={key}&value={value}'
